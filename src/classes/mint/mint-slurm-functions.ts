...
import { loadModelWCM, getModelCacheDirectory, queueModelExecutionsSlurm } from "../slurm/slurm-execution-functions";

import fs from "fs-extra";

import { DEVMODE } from "../../config/app";

export const saveAndRunExecutionsSlurm = async (
        thread: Thread, 
        modelid: string,
    ...
    for(let pmodelid in thread.model_ensembles) {
        if(!modelid || (modelid == pmodelid)) {
            await saveAndRunExecutionsForModelSlurm(pmodelid, thread, prefs);
            /*
            if(!DEVMODE) {
    ...
    console.log("Finished sending all executions for slurm execution");
}

export const saveAndRunExecutionsForModelSlurm = async(modelid: string, 
        thread: Thread, 
        prefs: MintPreferences) => {
    ...
    try {
        ...
        if(configs != null) {
            ...
            let summary = {
                ...
                failed_runs: 0,
                successful_runs: 0,
                workflow_name: "", // No workflow. Slurm execution
            ...
            for(let i=0; i<configs.length; i+= batchSize) {
                ...
                queueModelExecutionsSlurm(thread, modelid, component, executions_to_be_run, prefs);
            }
        }
...
export const deleteExecutableCacheSlurm = async(
    thread: Thread, 
    modelid: string,
    prefs: MintPreferences) => {
    ...
    for(let pmodelid in thread.model_ensembles) {
        if(!modelid || (modelid == pmodelid))
            await deleteExecutableCacheForModelSlurm(pmodelid, thread, prefs);
    }
    console.log("Finished deleting all execution cache for slurm execution");

    //monitorAllEnsembles(thread, problem_statement, prefs);
}

export const deleteModelInputCacheSlurm = (
    thread: Thread,
    modelid: string,
    prefs: MintPreferences) => {
    ...
    for(let dsid in thread.data) {
        ...
        ds.resources.map((res) => {
            let file = prefs.slurm.datadir + "/" + res.name;
            ...
            if(resources.length > 0) {
                ...
                resources.map((res) => {
                    if(res.url) {
                        ...
                        let file = prefs.slurm.datadir + "/" + filename;
                        if(fs.existsSync(file)) {
                            fs.remove(file)
                        }
                    }
...
export const deleteExecutableCacheForModelSlurm = async(modelid: string, 
    thread: Thread, 
    prefs: MintPreferences) => {
    ...
    deleteModelInputCacheSlurm(thread, modelid, prefs);
